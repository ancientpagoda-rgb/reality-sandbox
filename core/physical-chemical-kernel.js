...
const REACTIONS = Object.freeze([
  {
    id: 'water-rock-hydrogen-generation',
    reactants: { ferrousOxide: 3, water: 1 },
    products: { magnetite: 1, hydrogen: 1 },
    deltaG0: -42,
    activationEnergy: 68,
    prefactor: 2.4e9,
    mineralCatalysis: 18,
    hydrothermalCatalysis: 32,
  },
  {
    id: 'serpentinization-carbon-reduction',
    reactants: { carbonDioxide: 1, hydrogen: 4 },
    products: { methane: 1, water: 2 },
    deltaG0: -130,
    activationEnergy: 74,
    prefactor: 3.0e10,
    mineralCatalysis: 34,
    hydrothermalCatalysis: 18,
  },
  {
    id: 'energy-driven-carbon-fixation',
    reactants: { carbonDioxide: 1, hydrogen: 2 },
    products: { reducedCarbon: 1, water: 1 },
    deltaG0: 38,
    activationEnergy: 86,
    prefactor: 1.2e11,
    mineralCatalysis: 30,
    hydrothermalCatalysis: 22,
    energyCoupling: 72,
  },
  {
    id: 'abiotic-nitrogen-fixation',
    reactants: { nitrogen: 1, hydrogen: 3 },
    products: { ammonia: 2 },
    deltaG0: -33,
    activationEnergy: 118,
    prefactor: 2.0e12,
    mineralCatalysis: 42,
    ultravioletCatalysis: 25,
    energyCoupling: 20,
  },
  {
    id: 'carbon-nitrogen-coupling',
    reactants: { reducedCarbon: 1, ammonia: 1 },
    products: { formamide: 1, hydrogen: 1 },
    deltaG0: 24,
    activationEnergy: 82,
    prefactor: 1.8e11,
    mineralCatalysis: 24,
  ...
  {
    id: 'amphiphile-condensation',
    reactants: { reducedCarbon: 8, hydrogen: 6 },
    products: { amphiphile: 1, water: 6 },
    deltaG0: 96,
    activationEnergy: 82,
    prefactor: 8.0e11,
    mineralCatalysis: 38,
  ...
  {
    id: 'nitrogenous-condensation',
    reactants: { formamide: 2 },
    products: { condensate: 1, water: 1 },
    deltaG0: 31,
    activationEnergy: 91,
    prefactor: 6.0e11,
    mineralCatalysis: 20,
    wetDryCatalysis: 35,
    ultravioletCatalysis: 11,
    energyCoupling: 70,
  },
  {
    id: 'formamide-photolysis',
    reactants: { formamide: 1 },
    products: { carbonMonoxide: 1, ammonia: 1 },
    deltaG0: 18,
    activationEnergy: 76,
    prefactor: 7.0e10,
    ultravioletCatalysis: 48,
    energyCoupling: 58,
  },
  {
    id: 'condensate-hydrolysis',
    reactants: { condensate: 1, water: 1 },
    products: { formamide: 2 },
    deltaG0: -12,
    activationEnergy: 63,
    prefactor: 5.0e9,
    hydrothermalCatalysis: 10,
  },
  {
    id: 'amphiphile-hydrolysis',
    reactants: { amphiphile: 1, water: 6 },
    products: { reducedCarbon: 8, hydrogen: 6 },
    deltaG0: -8,
    activationEnergy: 104,
    prefactor: 4.0e9,
    hydrothermalCatalysis: 8,
    ultravioletCatalysis: 11,
  },
]);
...
export function createPhysicalChemicalKernel(options = {}) {
  ...
  const species = Object.fromEntries(
    Object.keys(SPECIES).map(name => [name, new Float64Array(size)]),
  );
  ...
  const local = {
    temperatureK: new Float32Array(size),
    pressureBar: new Float32Array(size),
    energyFlux: new Float32Array(size),
    redoxDrive: new Float32Array(size),
    organics: new Float32Array(size),
    amphiphiles: new Float32Array(size),
    precursors: new Float32Array(size),
    complexity: new Float32Array(size),
    reactionEnergyUse: new Float32Array(size),
    reactionHeat: new Float32Array(size),
  };

  const initialElementTotals = new Float64Array(8);
  ...
  function seedFromAstrophysics() {
    ...
    for (let index = 0; index < size; index++) {
      ...
      const temperatureK = clamp(baseTemperature + (normalizedTemperature - 0.58) * 125 + hydrothermal * 55, 165, 520);
      const pressureBar = clamp(0.03 + atmosphere ** 1.7 * (0.55 + (Number(planet.massEarth) || 1) ** 0.35), 0.02, 20);
      const energyFlux = clamp(hydrothermal * 0.58 + ultraviolet * 0.24 + wetDry * 0.28, 0, 1.8);

      local.temperatureK[index] = temperatureK;
      local.pressureBar[index] = pressureBar;
      local.energyFlux[index] = energyFlux;
      ...
      species.ammonia[index] = 0;
      species.methane[index] = 0;
      species.reducedCarbon[index] = 0;
      species.formamide[index] = 0;
      species.amphiphile[index] = 0;
      species.condensate[index] = 0;
    }
  }
  ...
  function step(dt) {
    ...
    for (let index = 0; index < size; index++) {
      for (const reaction of REACTIONS) applyReaction(index, reaction, simDt);
    }
  ...
  function applyReaction(index, reaction, dt) {
    ...
    const energyFlux = local.energyFlux[index];

    const catalystReduction =
      minerals * (reaction.mineralCatalysis || 0) +
      hydrothermal * (reaction.hydrothermalCatalysis || 0) +
      wetDry * (reaction.wetDryCatalysis || 0) +
      ultraviolet * (reaction.ultravioletCatalysis || 0);
    ...
    const coupledEnergy = energyFlux * (reaction.energyCoupling || 0);
    const effectiveDeltaG = deltaG - coupledEnergy;
    const thermodynamicDrive = effectiveDeltaG < 0
      ? clamp(1 - Math.exp(effectiveDeltaG / Math.max(EPSILON, R_KJ * temperature)), 0, 1)
      : 0;
    ...
    for (const [name, coefficient] of Object.entries(reaction.reactants)) {
      ...
      // Coarse-grained effective step: stoichiometric coefficients constrain
      // extent, while availability is per reactant pool rather than an
      // implausible simultaneous N-body collision term.
    ...
    const kineticExtent = arrhenius * availability * thermodynamicDrive * dt * 2.5e-6;
    const extent = Math.min(limitingExtent * 0.22, kineticExtent);
    ...
    for (const [name, coefficient] of Object.entries(reaction.reactants)) {
      species[name][index] = Math.max(0, species[name][index] - extent * coefficient);
    }
    for (const [name, coefficient] of Object.entries(reaction.products)) {
      species[name][index] += extent * coefficient;
    }
  }
  ...
  function reactionQuotient(index, reaction) {
    ...
    let denominator = 1;
    for (const [name, coefficient] of Object.entries(reaction.products)) {
      numerator *= Math.pow(activity(species[name][index]), coefficient);
    }
    for (const [name, coefficient] of Object.entries(reaction.reactants)) {
      denominator *= Math.pow(activity(species[name][index]), coefficient);
    }
    return numerator / Math.max(EPSILON, denominator);
  }

  function activity(amount) {
    return Math.max(EPSILON, amount / (1 + amount));
  }
  ...
  function refreshDerivedFields() {
    for (let index = 0; index < size; index++) refreshCell(index);
  }

  function refreshCell(index) {
    const reduced = species.reducedCarbon[index];
    const formamide = species.formamide[index];
    const amphiphile = species.amphiphile[index];
    const condensate = species.condensate[index];
    const phosphate = species.phosphate[index];
    const methane = species.methane[index];
    local.organics[index] = saturate(
      reduced * 0.52 + formamide * 0.9 + amphiphile * 2.8 + condensate * 1.35 + methane * 0.08,
      0.22,
    );
    local.amphiphiles[index] = saturate(amphiphile, 0.008);
    local.precursors[index] = saturate(
      Math.min(formamide * 1.7 + condensate * 2.2, phosphate * 120) + reduced * 0.14,
      0.055,
    );
  ...
  function chemicalDiversity(index) {
    const values = ['methane', 'reducedCarbon', 'formamide', 'amphiphile', 'condensate']
      .map(name => species[name][index])
  ...
  function consumeOrganics(index, normalizedAmount) {
    ...
    for (const name of pools) {
      ...
      species[name][index] = Math.max(0, species[name][index] - consumed * share);
    }
    refreshCell(index);
    return consumed;
  }
  ...
  function recalculateMetrics() {
    ...
    let complexity = 0;
    let energyFlux = 0;
    let reactionEnergyUse = 0;
    let reactionHeat = 0;
    let activeChemistry = 0;
    for (let index = 0; index < size; index++) {
      organics += local.organics[index];
      amphiphiles += local.amphiphiles[index];
      precursors += local.precursors[index];
      complexity += local.complexity[index];
      energyFlux += local.energyFlux[index];
      reactionEnergyUse += local.reactionEnergyUse[index];
      reactionHeat += local.reactionHeat[index];
      if (local.organics[index] > 0.02 && local.energyFlux[index] > 0.12) activeChemistry++;
    }
    metrics = {
      clock,
      organics: organics / size,
      amphiphiles: amphiphiles / size,
      precursors: precursors / size,
      complexity: complexity / size,
      energyFlux: energyFlux / size,
      reactionEnergyUse: reactionEnergyUse / size,
      reactionHeat: reactionHeat / size,
      activeChemistryCoverage: activeChemistry / size,
      maxElementDrift: calculateMaxElementDrift(),
    };
  }
    ...
    for (let index = 0; index < size; index++) {
      for (const [name, definition] of Object.entries(SPECIES)) {
        ...
        for (let e = 0; e < elementNames.length; e++) {
          target[e] += amount * (definition.formula[elementNames[e]] || 0);
        }
      }
    }
  }

  function sampleEnvironment(name, index, fallback) {
    const values = environment[name];
    if (!values || !Number.isFinite(values[index])) return fallback;
    return values[index];
  }
  ...
  function save() {
    return {
      ...
      species: Object.fromEntries(
        Object.keys(SPECIES).map(name => [name, Array.from(species[name], value => Math.round(value * 1e6) / 1e6)]),
      ),
    };
  }
  ...
  function load(state) {
    ...
    for (const name of Object.keys(SPECIES)) {
      ...
      for (let index = 0; index < Math.min(size, values.length); index++) {
        species[name][index] = Math.max(0, Number(values[index]) || 0);
      }
    }
...
function verifyReactionStoichiometry() {
  for (const reaction of REACTIONS) {
    const reactants = elementalBalance(reaction.reactants);
    const products = elementalBalance(reaction.products);
    ...
    for (const element of elements) {
      ...
      if (Math.abs(left - right) > 1e-9) {
        throw new Error(`Unbalanced physical-chemical reaction ${reaction.id}: ${element} ${left} != ${right}`);
      }
    }
  }
}
...
function elementalBalance(side) {
  ...
  for (const [name, coefficient] of Object.entries(side)) {
    ...
    for (const [element, count] of Object.entries(definition.formula)) {
...
function emptyMetrics() {
  return {
    ...
    reactionEnergyUse: 0,
    reactionHeat: 0,
    activeChemistryCoverage: 0,
    maxElementDrift: 0,
  };
}

function saturate(value, scale) {
  return clamp(value / Math.max(EPSILON, value + scale), 0, 1);
}
