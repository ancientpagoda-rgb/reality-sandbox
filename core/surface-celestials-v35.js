...
const FOV_DEG = 100;
const UPDATE_MS = 50;
const CELESTIAL_DAY_SECONDS = 900;
...
async function waitForRuntime() {
  for (let i = 0; i < 300; i++) {
    ...
    if (planet?.world && planet?.orbitalSystem?.getState && mode?.getPlayer && mode?.isActive && layer) {
...
function install({ planet, mode, layer }) {
  if (window.realitySandboxSurfaceCelestialsV35?.installed) return;
  ...
  const stars = Array.from({ length: 180 }, (_, i) => {
    ...
    return {
      ra,
      dec,
      alpha: 0.28 + (((h >>> 16) & 255) / 255) * 0.65,
      radius: 0.45 + ((h >>> 24) & 3) * 0.20,
    };
  });
  ...
  const stats = {
    ...
    moonIllumination: 0,
    moonPhase: 'unknown',
    sunAngularDiameterDeg: 0,
    moonAngularDiameterDeg: 0,
    daylight: 1,
    twilight: 0,
    visualDay: 0,
    visualRotationRate: 1 / CELESTIAL_DAY_SECONDS,
    orbitSteps: 0,
  };
  ...
  function localSky(vector, player, tilt, siderealDay) {
    ...
    return localSkyFromRaDec(ra, dec, player, siderealDay);
  }
  ...
  function moonAtVisualDay(state, deltaDays) {
    ...
    const angle = baseAngle + TAU * deltaDays / Math.max(1, moon.periodDays || 27.3);
    return {
  ...
  function angularSizes(state) {
    ...
    return {
      sun: clamp(sunRadius, 0.0012, 0.025),
      moon: clamp(moonRadius, 0.0015, 0.04),
    };
  }
  ...
  function drawMoon(x, y, r, illumination, waxing) {
    ...
    ctx.arc(x, y, r, 0, TAU);
    ctx.clip();
    ctx.fillStyle = 'rgba(205,209,207,.98)';
    ctx.fillRect(x - r, y - r, r * 2, r * 2);
    const dark = 1 - illumination;
    const shift = (waxing ? 1 : -1) * (2 * illumination - 1) * r;
    ctx.fillStyle = 'rgba(6,9,14,.97)';
    ctx.beginPath();
    ctx.ellipse(x + shift, y, Math.max(0.08, dark) * r * 1.98, r, 0, 0, TAU);
    ctx.fill();
    ctx.restore();
    ctx.strokeStyle = 'rgba(235,239,237,.72)';
    ctx.lineWidth = 0.7;
    ctx.beginPath();
    ctx.arc(x, y, r, 0, TAU);
    ctx.stroke();
  ...
  function draw(player, sunSky, moonSky, sizes, illumination, waxing) {
    ...
    // Only a neutral whole-scene night tint is screen-space. Spatial references
    // (stars, Sun and Moon) are all projected through the same yaw/pitch basis as
    // the surface camera, so the sky no longer appears pinned to the display.
    const nightAlpha = (1 - daylight) * (0.44 - twilight * 0.16);
    if (nightAlpha > 0.03) {
      ctx.fillStyle = `rgba(2,7,16,${clamp(nightAlpha, 0, 0.48)})`;
    ...
    if (starAlpha > 0.04) {
      for (const star of stars) {
        ...
        const p = projectSky(sky, player);
        if (!p?.visible) continue;
        ctx.fillStyle = `rgba(235,242,255,${star.alpha * starAlpha})`;
        ctx.beginPath();
        ctx.arc(p.x, p.y, star.radius, 0, TAU);
        ctx.fill();
      }
    }
    ...
    if (sp?.visible && sunSky.altitude > -0.12) {
      ...
      const glow = ctx.createRadialGradient(sp.x, sp.y, r * 0.15, sp.x, sp.y, r * 4.2);
      glow.addColorStop(0, 'rgba(255,247,196,.98)');
      glow.addColorStop(0.2, 'rgba(255,221,128,.50)');
      glow.addColorStop(1, 'rgba(255,205,110,0)');
      ctx.fillStyle = glow;
      ctx.beginPath();
      ctx.arc(sp.x, sp.y, r * 4.2, 0, TAU);
      ctx.fill();
      ctx.fillStyle = 'rgba(255,246,190,1)';
      ctx.beginPath();
    ...
    if (mp?.visible && moonSky.altitude > -0.10) {
      drawMoon(mp.x, mp.y, pxRadius(sizes.moon), illumination, waxing);
    }
  }

  function beginSurfaceEpoch() {
    epochState = orbitalSystem.getState();
    epochVisualDay = epochState.day;
    visualDay = epochVisualDay;
    stats.visualDay = visualDay;
  }

  function update() {
    resize();
    if (!epochState) beginSurfaceEpoch();
    const player = mode.getPlayer();
    const state = epochState;
    const sun = state.bodies.find(b => b.id === 'sun');
    if (!sun?.position || !state.moon?.position) return;

    const deltaDays = visualDay - epochVisualDay;
    const moonPosition = moonAtVisualDay(state, deltaDays);
    ...
    const ml = Math.hypot(moonPosition.x, moonPosition.y, moonPosition.z) || 1;
    const sl = Math.hypot(sun.position.x, sun.position.y, sun.position.z) || 1;
    const cosE = clamp(
      (moonPosition.x * sun.position.x + moonPosition.y * sun.position.y + moonPosition.z * sun.position.z) / (ml * sl),
      -1,
      1,
    );
    const illumination = (1 - cosE) * 0.5;
    const waxing = (sun.position.z * moonPosition.x - sun.position.x * moonPosition.z) >= 0;

    stats.sunAltitudeDeg = sunSky.altitude * 180 / Math.PI;
    stats.sunAzimuthDeg = wrapTau(sunSky.azimuth) * 180 / Math.PI;
    stats.moonAltitudeDeg = moonSky.altitude * 180 / Math.PI;
    stats.moonAzimuthDeg = wrapTau(moonSky.azimuth) * 180 / Math.PI;
  ...
  function loop(now) {
    ...
    last = now;
    if (!window.realitySandboxDebug?.isPaused?.()) {
      visualDay += dt / CELESTIAL_DAY_SECONDS;
    }
  ...
  const api = {
    ...
    getStats: () => ({
      ...
      source: 'Nysa orbital epoch + independent slow surface rotation clock',
      axialTiltCoupled: true,
      observerLatitudeLongitudeCoupled: true,
      orbitalClockRunsInSurface: false,
      surfaceRotationClockIndependent: true,
      celestialDaySeconds: CELESTIAL_DAY_SECONDS,
      respectsPause: true,
      moonPhaseFromElongation: true,
      apparentAngularSizeModeled: true,
      cameraProjectedStars: true,
      cameraProjectedSunMoon: true,
      screenFixedStarfield: false,
    }),
  };

  window.realitySandboxSurfaceCelestialsV35 = api;
  document.documentElement.dataset.surfaceCelestialsV35 = 'stable-camera-projected-orbital-sky';

  const prev = window.realitySandboxPresentationDiagnostics;
  window.realitySandboxPresentationDiagnostics = () => ({
    ...(typeof prev === 'function' ? prev() : {}),
    surfaceCelestialsV35: api.getStats(),
  });
}

waitForRuntime().then(state => {
  if (!state) {
    document.documentElement.dataset.surfaceCelestialsV35 = 'unavailable';
    return;
  }
  install(state);
});
