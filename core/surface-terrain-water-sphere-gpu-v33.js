...
function install({ planet, modules, mode, layer, inputCanvas }) {
  if (window.realitySandboxSurfaceSphereV33?.installed) return;
  ...
  try {
    ...
    document.documentElement.dataset.surfaceGpu = 'sphere-v33-webgl-unavailable';
    return;
  }
  ...
  renderer.domElement.setAttribute('aria-label', 'Curved cached GPU terrain and water surface view of Nysa');
  ...
  const waterMaterial = new THREE.ShaderMaterial({
    uniforms: {
      time: { value: 0 }, deepColor: { value: new THREE.Color(0x0b466e) }, shallowColor: { value: new THREE.Color(0x2f91aa) },
    },
    transparent: true,
    depthWrite: false,
  ...
  renderer.domElement.addEventListener('webglcontextlost', event => {
    ...
    document.documentElement.dataset.surfaceGpu = 'sphere-v33-context-lost';
  }, false);
  ...
  const gpuApi = {
    ...
    getStats: () => ({
      ...
      gpuPrimary: true,
      active: isPresenting(),
      diagnosticScene: 'cached-spherical-terrain-water-lod-rings',
      frames: stats.frames,
  ...
  const api = {
    ...
    getStats: () => ({
      ...
      scene: 'cached-spherical-terrain-water-lod-rings',
  ...
  window.realitySandboxSurfaceSphereV33 = api;
  document.documentElement.dataset.surfaceSphereV33 = 'curved-water-two-lod-rings';
...
async function boot() {
  ...
  if (!state) {
    document.documentElement.dataset.surfaceSphereV33 = 'unavailable';
    return;
  }
  install(state);
}
