...
const SENSOR_LABELS = {
  ...
  preyOpportunity: 'prey opportunity',
  threat: 'threat',
  kinDensity: 'kin density',
  sleepDebt: 'sleep debt',
  soilToxin: 'soil toxin',
  habitatStress: 'habitat stress',
};
...
function install({ brains, inspector, root }) {
  ...
  if (bodyHistory) bodyHistory.insertAdjacentElement('afterend', box);
  else root.querySelector('.body-plan-v48')?.insertAdjacentElement('afterend', box);

  let renders = 0;
  let lastSelected = null;

  function strongestLinks(brain, action) {
    const weights = brain?.weights?.[action] || {};
    const ranked = Object.entries(weights).sort((a, b) => Math.abs(b[1]) - Math.abs(a[1]));
    const positive = ranked.find(([, value]) => value > 0.02) || null;
    const negative = ranked.find(([, value]) => value < -0.02) || null;
    return { positive, negative };
  }

  function render() {
    const selected = inspector.getStats?.().selectedLineageId;
    const lineages = brains.getLineageBrains?.() || [];
    const lineage = selected ? lineages.find(item => item.lineageId === selected) : null;
    const summary = box.querySelector('.brain-summary-v50a');
    const actions = box.querySelector('.brain-actions-v50a');
    const wires = box.querySelector('.brain-wires-v50a');
    if (!summary || !actions || !wires) return;

    if (!lineage?.brain) {
      summary.innerHTML = '';
      actions.innerHTML = '';
      wires.innerHTML = '<div class="brain-empty-v50a">No living motile brain is available for this lineage.</div>';
    ...
    summary.innerHTML = [
      [lineage.population, 'brains sampled'],
      [pct(brain.memoryStrength), 'action memory'],
      [pct(brain.decisionRate), 'decision rate'],
      [pct(brain.inhibition), 'inhibition'],
      [brains.sensors.length, 'sensors'],
      [brains.actions.length, 'actions'],
    ].map(([value, label]) => `<div><b>${value}</b><span>${label}</span></div>`).join('');

    const totalActions = Object.values(lineage.currentActions || {}).reduce((sum, value) => sum + (Number(value) || 0), 0) || 1;
    actions.innerHTML = brains.actions.map(action => {
      const count = Number(lineage.currentActions?.[action]) || 0;
      return `<div><b>${Math.round((count / totalActions) * 100)}%</b><span>${ACTION_LABELS[action] || action}</span></div>`;
    }).join('');
  ...
  const api = {
    ...
    getStats: () => ({
      ...
      actionDistribution: true,
      sensorActionWeights: true,
      strongestConnectionView: true,
