...
function install({ language, planet, modules }) {
  ...
  function receiverRootAnchor(organism, meaning, tokens) {
    const state = organism.bioV54;
    if (!state) return null;
    let best = null;
    let confidence = 0;
    for (const token of tokens) {
      ...
      if (entry?.meaning === meaning && entry.confidence >= 0.34 && entry.confidence > confidence) {
        best = token;
  ...
  function modifierTokenFor(id, state, modifier, rootToken) {
    const known = bestTokenForPrimitive(state, modifier);
    if (known && MODIFIER_TOKENS.includes(known) && known !== rootToken) return known;
  ...
  function ensureSyntax(id, state) {
    ...
    state.syntaxOrder = (hash32(`${seed}:${id}:syntax-v55b`) & 1) ? 'referent-modifier' : 'modifier-referent';
  ...
  function learnFromGroundedPhrase(receiver, state, emission, reception) {
    const groundedPair = inferPair(receiver);
    const groundedMeaning = meaningForPair(groundedPair);
    if (!groundedPair || !groundedMeaning) return false;
    const rootToken = receiverRootAnchor(receiver, groundedMeaning, emission.tokens);
    if (!rootToken) return false;
    const rootIndex = emission.tokens.indexOf(rootToken);
    if (rootIndex < 0) return false;
    const modifierToken = emission.tokens[rootIndex === 0 ? 1 : 0];
    if (!MODIFIER_TOKENS.includes(modifierToken)) return false;

    associatePrimitive(state, rootToken, groundedPair.referent, reception, emission.emitterId);
    associatePrimitive(state, modifierToken, groundedPair.modifier, reception, emission.emitterId);
    learnSyntaxFromRootPosition(state, rootIndex, reception);
    ...
    for (const emission of emissions) {
      ...
      for (const key of neighborKeys(emitterPos.x, emitterPos.y, rings)) {
        for (const receiverId of grid.get(key) || []) {
          ...
          if (learnFromGroundedPhrase(receiver, state, emission, reception)) stats.groundedPhraseHearings++;
          else stats.ungroundedPhraseHearings++;
  ...
  modules.step = function v55bGroundedCompositionStep(dt) {
  ...
  const api = {
    ...
    getStats:() => ({
      ...stats,
      installed:true,
      version:'v55b-holophrase-reanalysis',
  ...
  document.documentElement.dataset.compositionalLanguageV55 = 'grounded-holophrase-reanalysis';
}
