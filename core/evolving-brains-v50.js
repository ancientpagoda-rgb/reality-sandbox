...
function makeWeights() {
  return Object.fromEntries(ACTIONS.map(action => [action, Object.fromEntries(SENSORS.map(sensor => [sensor, 0]))]));
}

function copyBrain(brain) {
  const weights = makeWeights();
  for (const action of ACTIONS) {
    for (const sensor of SENSORS) weights[action][sensor] = clampSigned(brain?.weights?.[action]?.[sensor]);
  }
  return {
...
function mutateBrain(parent, seed, scale = 0.075) {
  ...
  for (const action of ACTIONS) {
    ...
    for (const sensor of SENSORS) {
      const delta = (hash01(`${seed}:${action}:${sensor}`) - 0.5) * scale * 2;
      brain.weights[action][sensor] = clampSigned(brain.weights[action][sensor] + delta);
    }
  }
...
async function waitForRuntime() {
  while (true) {
    ...
    if (origin?.installed && inheritance?.installed && nutrients?.installed && components?.motile instanceof Map && modules?.step) {
      return { origin, nutrients, planet, modules };
    }
...
function install({ origin, nutrients, planet, modules }) {
  ...
  let accumulator = 0;
  let prototypeAccumulator = 0;
  let decisions = 0;
  let sensorEvaluations = 0;
  let spatialGridBuilds = 0;
  let brainsAssigned = 0;
  let lineageInheritedBrains = 0;
  let parentLineageInheritedBrains = 0;
  let founderBrains = 0;
  let prototypeRefreshes = 0;

  function cellKey(x, y) {
    return `${Math.floor(wrap(x, world.width) / CELL_SIZE)}:${Math.floor(wrap(y, world.height) / CELL_SIZE)}`;
  }

  function neighborKeys(x, y) {
    const cx = Math.floor(wrap(x, world.width) / CELL_SIZE);
    const cy = Math.floor(wrap(y, world.height) / CELL_SIZE);
    const nx = Math.max(1, Math.ceil(world.width / CELL_SIZE));
    const ny = Math.max(1, Math.ceil(world.height / CELL_SIZE));
    const keys = [];
    for (let oy = -1; oy <= 1; oy++) {
      for (let ox = -1; ox <= 1; ox++) keys.push(`${(cx + ox + nx) % nx}:${(cy + oy + ny) % ny}`);
    }
    return keys;
  }
  ...
  function refreshPrototypes() {
    ...
    for (const [, organism] of motile.entries()) {
      ...
      let bucket = buckets.get(organism.lineageId);
      if (!bucket) buckets.set(organism.lineageId, bucket = []);
      bucket.push(organism.brainV50.genome);
    }
    for (const [lineageId, brains] of buckets) {
      const mean = averageBrains(brains);
      if (mean) lineagePrototypes.set(lineageId, mean);
    }
    prototypeRefreshes++;
  }

  function parentPrototype(lineageId) {
    const direct = lineagePrototypes.get(lineageId);
    if (direct) return { brain: direct, mode: 'lineage' };
    ...
    for (let index = ancestry.length - 1; index >= 0; index--) {
      ...
      const parent = lineagePrototypes.get(parentId);
  ...
  function assignBrain(id, organism) {
    ...
    if (inherited?.mode === 'lineage') lineageInheritedBrains++;
    else if (inherited?.mode === 'parent-lineage') parentLineageInheritedBrains++;
    else founderBrains++;
    organism.brainV50 = {
      genome: brain,
      lastAction: 'explore',
      activation: Object.fromEntries(ACTIONS.map(action => [action, 0])),
      sensors: Object.fromEntries(SENSORS.map(sensor => [sensor, 0])),
      cooldown: 0,
      decisionCount: 0,
      inheritedBy: inherited?.mode || 'founder',
      parentLineageId: inherited?.parentId || null,
    };
    if (!lineagePrototypes.has(lineageId)) lineagePrototypes.set(lineageId, copyBrain(brain));
    brainsAssigned++;
  }

  // Existing motiles predate this module; future births go through the wrapped set.
  for (const [id, organism] of motile.entries()) assignBrain(id, organism);
  refreshPrototypes();

  const nativeSet = motile.set.bind(motile);
  motile.set = function v50BrainInheritance(id, organism) {
    const result = nativeSet(id, organism);
  ...
  function nearestFood(p, resourceGrid, detritusGrid, radius, genome) {
    ...
    for (const key of neighborKeys(p.x, p.y)) {
      for (const id of resourceGrid.get(key) || []) {
        ...
        if (d < bestD) { bestD = d; best = { id, p: rp, d, kind: 'plant', amount: res.amount || 0 }; }
      }
      if (clamp(genome.scavenging) > 0.18) {
        for (const id of detritusGrid.get(key) || []) {
          const det = detritus.get(id);
          const dp = position.get(id);
          if (!det || !dp || (det.amount || 0) <= 0.02) continue;
          const d = distance(p, dp, world);
          if (d < bestD) { bestD = d; best = { id, p: dp, d, kind: 'detritus', amount: det.amount || 0 }; }
        }
      }
    }
    return best;
  ...
  function sensorsFor(id, organism, p, nearby, resourceGrid, detritusGrid) {
    ...
    const senseRadius = 38 + clamp(genome.sense) * 160;
    ...
    const ownSize = clamp(genome.bodySize);
    const ownArmor = clamp(genome.armor);
    for (const other of nearby) {
      const otherGenome = other.organism.genome || {};
      if (other.organism.lineageId === organism.lineageId) {
        kin++;
        kinX += wrappedDelta(other.p.x, p.x, world.width);
        kinY += wrappedDelta(other.p.y, p.y, world.height);
        continue;
      }
      const otherSize = clamp(otherGenome.bodySize);
      const otherAggression = clamp(otherGenome.aggression);
      ...
      if (preyScore > 0.40) {
        prey += preyScore;
        if (!nearestPrey || other.d < nearestPrey.d) nearestPrey = other;
      }
      if (threatScore > 0.38) {
        threat += threatScore;
        if (!nearestThreat || other.d < nearestThreat.d) nearestThreat = other;
      }
    }

    const nutrient = nutrients.sample(p.x, p.y);
    const habitatFitness = clamp(organism.bioV48?.habitatFitness ?? 0.55);
    const foodAvailability = food ? clamp((food.amount || 0) * (1 - food.d / Math.max(1, senseRadius))) : 0;
    const sensors = {
      energyLow: clamp(1 - (Number(organism.energy) || 0) / 2.7),
      foodScarcity: clamp(1 - foodAvailability),
      preyOpportunity: clamp(prey / 2.2),
      threat: clamp(threat / 2.0),
      kinDensity: clamp(kin / 7),
      sleepDebt: clamp((Number(organism.sleepDebt) || 0) / 1.2),
      soilToxin: clamp(nutrient?.toxin),
      habitatStress: clamp(1 - habitatFitness),
    };
    sensorEvaluations++;
    return {
      sensors,
      ...
      nearestPrey,
      nearestThreat,
      kinCenter: kin ? { x: wrap(p.x + kinX / kin, world.width), y: wrap(p.y + kinY / kin, world.height) } : null,
      senseRadius,
    };
  }

  function activationFor(brainState, organism, sensors) {
    const brain = brainState.genome;
    const genome = organism.genome || {};
    const phenotype = organism.bioV48?.phenotype || {};
    const activation = {};
    const traitBias = {
      ...
      flock: clamp(genome.sociality) * 0.30,
      rest: clamp(genome.dormancy) * 0.24,
  ...
  function steerToward(vel, p, target, speed, blend = 0.72) {
    if (!target) return;
    const dx = wrappedDelta(target.x, p.x, world.width);
    const dy = wrappedDelta(target.y, p.y, world.height);
  ...
  function steerAway(vel, p, target, speed, blend = 0.62) {
    if (!target) return;
    const dx = wrappedDelta(p.x, target.x, world.width);
    const dy = wrappedDelta(p.y, target.y, world.height);
    const d = Math.hypot(dx, dy) || 1;
    vel.vx = vel.vx * blend + (dx / d) * speed * (1 - blend);
    vel.vy = vel.vy * blend + (dy / d) * speed * (1 - blend);
  }
  ...
  function applyAction(id, organism, p, vel, action, context) {
    const genome = organism.genome || {};
    const neural = clamp(organism.bioV48?.phenotype?.neuralComplexity ?? genome.brainSpeed);
    const speed = 5 + clamp(genome.motility) * 31 + clamp(organism.bioV48?.phenotype?.contractility) * 7;
    switch (action) {
      case 'forage':
        if (context.food) steerToward(vel, p, context.food.p, speed, 0.68 - neural * 0.12);
        break;
      case 'hunt':
        if (context.nearestPrey) steerToward(vel, p, context.nearestPrey.p, speed * 1.08, 0.64 - neural * 0.14);
        break;
      case 'flee':
        if (context.nearestThreat) steerAway(vel, p, context.nearestThreat.p, speed * 1.12, 0.56 - neural * 0.10);
        else {
          const angle = hash01(`${world.seed}:brain-flee:${id}:${Math.floor(world.tick / 12)}`) * Math.PI * 2;
          vel.vx += Math.cos(angle) * speed * 0.16;
          vel.vy += Math.sin(angle) * speed * 0.16;
        }
        break;
      case 'flock':
        if (context.kinCenter) steerToward(vel, p, context.kinCenter, speed * 0.76, 0.84 - clamp(genome.sociality) * 0.12);
        break;
      case 'rest':
        if ((organism.energy || 0) > 0.28 && (organism.sleepDebt || 0) > 0.34 && context.sensors.threat < 0.42) {
      ...
      case 'explore': {
        ...
        vel.vx = vel.vx * 0.82 + Math.cos(angle) * speed * 0.18;
        vel.vy = vel.vy * 0.82 + Math.sin(angle) * speed * 0.18;
        break;
      }
    }
  }
  ...
  function decide(dt) {
    ...
    for (const [id, organism] of motile.entries()) {
      assignBrain(id, organism);
      const brainState = organism.brainV50;
      const p = position.get(id);
      const vel = velocity.get(id);
      if (!brainState || !p || !vel) continue;
      brainState.cooldown -= dt;
      if (brainState.cooldown > 0) continue;

      const genome = organism.genome || {};
      const senseRadius = 38 + clamp(genome.sense) * 160;
      const nearby = nearbyMotiles(id, p, motileGrid, senseRadius);
      const context = sensorsFor(id, organism, p, nearby, resourceGrid, detritusGrid);
      const activation = activationFor(brainState, organism, context.sensors);
      let chosen = 'explore';
      let best = -Infinity;
      for (const action of ACTIONS) {
        if (activation[action] > best) { best = activation[action]; chosen = action; }
      }

      brainState.sensors = { ...context.sensors };
      brainState.activation = activation;
      brainState.lastAction = chosen;
      brainState.decisionCount++;
      brainState.cooldown = Math.max(0.16, 1.25 - brainState.genome.decisionRate * 0.92);
      applyAction(id, organism, p, vel, chosen, context);
      actionCounts[chosen]++;
      decisions++;
    }
  }
  ...
  modules.step = function v50EvolvingBrainStep(dt) {
    ...
    accumulator += elapsed;
    prototypeAccumulator += elapsed;
    if (prototypeAccumulator >= PROTOTYPE_REFRESH_SECONDS) {
      prototypeAccumulator %= PROTOTYPE_REFRESH_SECONDS;
      refreshPrototypes();
    }
    if (accumulator >= STEP_SECONDS) {
      const decisionDt = accumulator;
      accumulator = 0;
      decide(decisionDt);
    }
    return result;
  ...
  function getBrain(id) {
    const state = motile.get(id)?.brainV50;
    if (!state) return null;
    return {
      genome: copyBrain(state.genome),
      lastAction: state.lastAction,
      activation: { ...state.activation },
      sensors: { ...state.sensors },
      decisionCount: state.decisionCount,
      inheritedBy: state.inheritedBy,
  ...
  function getLineageBrains() {
    ...
    for (const [id, organism] of motile.entries()) {
      if (!organism.lineageId || !organism.brainV50?.genome) continue;
      let bucket = buckets.get(organism.lineageId);
      if (!bucket) buckets.set(organism.lineageId, bucket = { brains: [], actions: Object.fromEntries(ACTIONS.map(action => [action, 0])), population: 0 });
      bucket.brains.push(organism.brainV50.genome);
      bucket.actions[organism.brainV50.lastAction] = (bucket.actions[organism.brainV50.lastAction] || 0) + 1;
      bucket.population++;
    }
    return [...buckets.entries()].map(([lineageId, bucket]) => ({
      lineageId,
      population: bucket.population,
      brain: averageBrains(bucket.brains),
      currentActions: { ...bucket.actions },
    }));
  }
  ...
  const api = {
    ...
    getStats: () => ({
      ...
      spatialGridBuilds,
      brainsAssigned,
      lineageInheritedBrains,
      parentLineageInheritedBrains,
      founderBrains,
      prototypeRefreshes,
      lineagePrototypes: lineagePrototypes.size,
      actionCounts: { ...actionCounts },
      sensors: SENSORS.slice(),
      actions: ACTIONS.slice(),
      inheritedSensorActionWeights: true,
      behaviorAffectsMovementAndRest: true,
      behaviorSelectionThroughEcology: true,
      spatiallyHashedSensing: true,
