...
function install(parts) {
  ...
  function invariants() {
    ...
    if (!state.compositionalLanguage.independentPrimitiveMeanings || !state.compositionalLanguage.compositionalGeneralization || !state.compositionalLanguage.learnedWordOrder || !state.compositionalLanguage.wordOrderConstrainsDecoding || !state.compositionalLanguage.syntaxLearnedFromObservedSequence || !state.compositionalLanguage.holophraseReanalysis || !state.compositionalLanguage.receiverGroundedPrimitiveLearning || !state.compositionalLanguage.rootAnchoredSyntaxLearning || !state.compositionalLanguage.noSpeakerSemanticMetadata || !state.compositionalLanguage.retainedCulturalKnowledgeCanBeComposed || !state.compositionalLanguage.culturallyBlankCompositionalLexiconAtBirth || !state.compositionalLanguage.nonGeneticCompositionalTransmission || !state.compositionalLanguage.physicallyLocalTransmission || !state.compositionalLanguage.kinBiasedTransmission || !state.compositionalLanguage.boundedPrimitiveLexicon || !state.compositionalLanguage.constantPairMemory || !state.compositionalLanguage.spatialHashing) failures.push('The v55 grounded compositional-language phenotype is incomplete.');
    if (state.compositionalLanguage.version !== 'v55b-holophrase-reanalysis') failures.push(`Unexpected v55 language version ${state.compositionalLanguage.version || 'unknown'}.`);
  ...
  document.documentElement.dataset.evolutionDiagnosticsV48d = 'ready-v55b-grounded';
