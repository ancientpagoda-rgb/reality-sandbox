...
(async () => {
  ...
  try {
    ...
    assert(after.build === 'surface-v46e-clean-no-fauna', `Unexpected build ${after.build}`);
    assert(after.faunaPolicy === 'disabled-pending-redesign', `Unexpected fauna policy ${after.faunaPolicy}`);
    ...
    await page.screenshot({ path: path.join(artifactDir, 'surface-mode-v46e-clean-no-fauna.png'), fullPage: true });
