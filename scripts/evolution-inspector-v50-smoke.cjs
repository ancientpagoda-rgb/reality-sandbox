...
(async () => {
  ...
  try {
    ...
    await page.waitForFunction(() => Boolean(
      ...
      window.realitySandboxEvolvingBrainsV50?.installed &&
      window.realitySandboxBrainInspectorV50a?.installed
    ), null, { timeout: 120000 });
    ...
    const inspected = await page.evaluate(() => {
      ...
      const lineageBrains = brains.getLineageBrains();
      return {
        ...
        brains: brains.getStats(),
        brainInspector: brainInspector.getStats(),
        lineagePhenotypes,
        selectedPhenotype: selected ? lineagePhenotypes.find(x => x.lineageId === selected) || null : null,
        lineageBrains,
        selectedBrain: selected ? lineageBrains.find(x => x.lineageId === selected) || null : null,
        ...
        brainSummaryCells: root?.querySelectorAll('.brain-summary-v50a > div')?.length || 0,
        brainActionCells: root?.querySelectorAll('.brain-actions-v50a > div')?.length || 0,
        brainWireRows: root?.querySelectorAll('.brain-wire-v50a')?.length || 0,
        brainText: root?.querySelector('.brain-v50a')?.textContent || '',
        lineages,
    ...
    assert(inspected.evolutionBuild === 'evolution-v50-evolving-brains', `Unexpected evolution build ${inspected.evolutionBuild}`);
    ...
    assert(inspected.brains.installed && inspected.brains.authoritativeFixedStep && inspected.brains.inheritedSensorActionWeights, 'v50 evolving-brain system is not active.');
    assert(inspected.brains.sensors.length === 8 && inspected.brains.actions.length === 6, 'v50 brain schema is not 8 sensors × 6 actions.');
    assert(inspected.brains.decisions > 0 && inspected.brains.spatiallyHashedSensing, 'v50 brains did not make spatially grounded decisions.');
    assert(inspected.brainInspector.actionDistribution && inspected.brainInspector.sensorActionWeights && inspected.brainInspector.strongestConnectionView && inspected.brainInspector.surfaceRendererTouched === false, 'v50 brain-inspector contract is incomplete.');
    ...
    if (inspected.originStats.motileLineages >= 1) {
      ...
      assert(inspected.selectedBrain && inspected.selectedBrain.population >= 1 && inspected.selectedBrain.brain, 'Selected motile lineage has no averaged v50 brain.');
      assert(Object.keys(inspected.selectedBrain.brain.bias || {}).length === 6 && Object.keys(inspected.selectedBrain.brain.weights || {}).length === 6, 'Selected v50 lineage brain is incomplete.');
      assert(Object.values(inspected.selectedBrain.brain.weights || {}).every(row => Object.keys(row).length === 8), 'Selected v50 lineage brain lacks its complete sensor-action matrix.');
      assert(inspected.brainSummaryCells === 6, `Expected 6 v50 brain summary cells, found ${inspected.brainSummaryCells}.`);
      assert(inspected.brainActionCells === 6, `Expected 6 v50 action-distribution cells, found ${inspected.brainActionCells}.`);
      assert(inspected.brainWireRows === 6, `Expected 6 v50 sensor-action connection rows, found ${inspected.brainWireRows}.`);
      assert(inspected.brainText.includes('Evolving brain') && inspected.brainText.includes('Forage') && inspected.brainText.includes('Explore'), 'v50 evolving-brain inspector did not render its behavior network.');
    }
    assert(pageErrors.length === 0, `Browser errors: ${pageErrors.join(' | ')}`);

    fs.writeFileSync(path.join(artifactDir, 'evolution-inspector.json'), JSON.stringify({ inspected, pageErrors }, null, 2));
    await page.screenshot({ path: path.join(artifactDir, 'evolution-inspector-v50.png'), fullPage: true });
  } finally {
    await browser.close();
  }
